function tgzProcess_Test(fnms, outDir)
...
%     end
    ...
    %fnmx = fnms{i};
    clear answer;
    [a1 b1 c1] = spm_fileparts(fnms{i});
    answer{1} = ['M' b1(end-3:end)]
    if (b1(end-4) == 'c')
        answer{2} = ['LARC']
        %fnmx = b1(1:end-8);
    elseif (b1(end-4) == 'r')
...
for i = 1 : numel(fnms)
    ...
    outDirx = outDir;
    if (b1(end-4) == 'c')
        outDirx = outDirx(1:end-8);
        %fnmx = b1(1:end-8);
    elseif (b1(end-4) == 'r')
        outDirx = outDirx(1:end-9);
        %fnmx = b1(1:end-9);
    elseif (b1(end-4) == 'e')
        outDirx = outDirx(1:end-8);
        %fnmx = b1(1:end-8)
    end;
    outDirN = fullfile(outDirx,ids{i});
    ...
    if isfolder(fnm)
        ...
        fprintf('Running \n %s\n', cmd);
        system(cmd,'-echo');
    elseif strcmpi(x,'.tgz') || strcmpi(x,'.tar.gz')
